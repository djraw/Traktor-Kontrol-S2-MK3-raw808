...
TraktorS2MK3.padModeHandler = function (field) {
    ...
    if (TraktorS2MK3.padModeState[field.group] === 0 && field.name === "!samples") {
        ...
        for (var i = 1; i <= 8; ++i) {
            ...
            // R.A.W. added colored hotcues
            var active = engine.getValue(field.group, "hotcue_" + i + "_enabled") + (i * 8 + 1);
...
TraktorS2MK3.jogTouchHandler = function (field) {
    ...
    if (field.value > 0) {
        // R.A.W. changed resolution of the MIDI control (in intervals per revolution) from 1024 to 768 (faster)
        engine.scratchEnable(deckNumber, 768, 33 + 1 / 3, 0.125, 0.125 / 8, true);
    } else {
...
TraktorS2MK3.outputHandler = function (value, group, key) {
    ...
    if (value === 0 || value === false) {
        ...
        // R.A.W. added colored hotcues
        if (key.match(/hotcue_.*/)) {
            var cuenum = parseInt(key.substr(7, 1));
            //var color = engine.getValue("[Channel1]", "hotcue_" + cuenum + "_enabled") + (cuenum * 4 + 1);
            //ledValue = color * 2 * 4 + 1;
            //ledValue = 134;
            ledValue = engine.getValue("[Channel1]", "hotcue_" + cuenum + "_enabled") + (cuenum * 8 + 1);
        }
    }
...
TraktorS2MK3.samplesOutputHandler = function (value, group, key) {
    ...
    if (TraktorS2MK3.padModeState[deck] === 1) {
        if (key === "play" && engine.getValue(group, "track_loaded")) {
            if (value) {
                // Green light on play - default old 0x9E
                //TraktorS2MK3.outputHandler(0x9E, deck, "pad_" + num);

                // 0x7C - dark white, 0x7E - white
                // 0x2E - blue, 0x3E - pink, 0x8E - orange, 0x9E - green, A mid, B bright, E dark
                // 32 - dark mint, 34 - mint, 46 - blue, 47 - light blu, 48 - dark purple, 49 - mid purple, 50 bright purple, 52 54 - purple
                // 64 dark red, 132 - dark red, 133 - mid red,  134 - red, 135 light red, 136 dark orange, 137 dark red, 138 mid orange
                // 139 light pink, 142 slight bright orange, 143 very light pink, 158 - green, 159 light green
                if (num == 1) {
                    led = 34
                }
                else if (num == 2) {
                ...
                TraktorS2MK3.outputHandler(led, deck, "pad_" + num);
            } else {
...
TraktorS2MK3.lightDeck = function (switchOff) {
    ...
    for (var i = 1; i <= 8; ++i) {
        // R.A.W. added colored hotcues
        //var color = engine.getValue("[Channel1]", "hotcue_" + i + "_enabled") + (i * 8 + 1);
        if (!switchOff) {fullLight = engine.getValue("[Channel1]", "hotcue_" + i + "_enabled") + (i * 8 + 1); };
        current = (!!engine.getValue("[Channel1]", "hotcue_" + i + "_enabled")) ? fullLight : softLight;
        //current = (!!engine.getValue("[Channel1]", "hotcue_" + i + "_enabled")) ? color : softLight;
        ...
        //current = (!!engine.getValue("[Channel2]", "hotcue_" + i + "_enabled")) ? color : softLight;
